...
import { useNftBannerConfig } from './hooks/useNftBannerConfig'
...
const NftBanner: React.FC<React.PropsWithChildren> = () => {
  const bannerList = useNftBannerConfig()
    ...
    <BannerPlaceHolder walletConnected={Boolean(account) && chainId === ChainId.BSC} style={{padding: 0, margin: 0}}>
      ...
      >
        {bannerList.map((banner, index) => {
          ...
          return (
            <SwiperSlide style={{ padding: isDesktop || isTablet ? 20 : 0, width: '50px' }} key={childKey}>
              {banner}
            </SwiperSlide>
          )
        })}
